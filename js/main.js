...
function renderBoard(board) {
    ...
    for (var i = 0; i < board.length; i++) {
        ...
        for (var j = 0; j < row.length; j++) {
            ...
            var tdId = `cell-${i}-${j}`;
                                                            // Added (the event passing) 21.5.20 to stop context menu from opening.
            strHtml += `<td id="${tdId}" class="cell hide" oncontextmenu="cellMarked(this, ${i}, ${j}, event)" onclick="cellClicked(this, ${i}, ${j})">
                            ${cell.element}
                        </td>`
        }
        strHtml += '</tr>';
...
function cellMarked(elCell, i, j, event) { // Added (the event passing) 21.5.20 to stop context menu from opening.
    event.preventDefault(); // Added 21.5.20 to stop context menu from opening.
    if (gIsProcessing === true) return;
    ...
    for (var i = x - 1; i <= x + 1; i++) {
        for (var j = y - 1; j <= y + 1; j++) {
            ...
            if (cell.isMine === false) {
                if (cell.isShown === true) continue; // flags an already 'done' cell and prevents the recursion's crash (!) July 2020.
                var cellSelector = getCellId({ i: i, j: j })
                var elCell = document.querySelector('#' + cellSelector);
                ...
                if (cell.minesAroundCount === 0) {expandShown(board, i, j)}; // The recursion was finally tweaked at July 2020.
            } else continue;
        }
    }
}
...
function setHints() {
    ...
    var threeHints = safes.slice(0, 3);
    for (var i = 0; i < threeHints.length; i++) {
        gHints.push(threeHints[i]);
    }
}
...
function giveHint() {
    ...
    if (gGame.hints === 3) {
        gGame.hints--;
        var hint1 = document.querySelector('#' + gHints[0]);
        hint1.classList.add('hint');
        setTimeout(function () { hint1.classList.remove('hint'); }, 3000);
        renderHints();
    }
    else if (gGame.hints === 2) {
        gGame.hints--;
        var hint2 = document.querySelector('#' + gHints[1]);
        hint2.classList.add('hint');
        setTimeout(function () { hint2.classList.remove('hint'); }, 3000);
        renderHints();
    } else if (gGame.hints === 1) {
        gGame.hints--;
        var hint3 = document.querySelector('#' + gHints[2]);
        hint3.classList.add('hint');
        setTimeout(function () { hint3.classList.remove('hint'); }, 3000);
        renderHints();
    } else return;
}
...
function hintNegs(board, elCell, x, y) {
    ...
    for (var i = x - 1; i <= x + 1; i++) {
        for (var j = y - 1; j <= y + 1; j++) {
            ...
            if (cell.isMine === true) {
                renderCell({ i: i, j: j }, MINE_IMG); // Replaced the hint of a mine with an Image. second param was originally 'M'. Changed July 2020
            } else {
                renderCell({ i: i, j: j }, cell.minesAroundCount);
            }
        }
...
function renderScores() {
    // Got rid of showing "null" results when no player had played. July 2020.
    if (localStorage.getItem('begName') !== null) {
        document.querySelector('#beginner').innerHTML = `${localStorage.getItem('begName')} with ${localStorage.getItem('beginnerTime')} seconds`;
    } else { document.querySelector('#beginner').innerHTML = 'No winners yet!'}
    if (localStorage.getItem('medName') !== null) {
        document.querySelector('#medium').innerHTML = `${localStorage.getItem('medName')} with ${localStorage.getItem('mediumTime')} seconds`;
    } else { document.querySelector('#medium').innerHTML = 'No winners yet!'}
    if (localStorage.getItem('expName') !== null) {
        document.querySelector('#expert').innerHTML = `${localStorage.getItem('expName')} with ${localStorage.getItem('expertTime')} seconds`;
    } else { document.querySelector('#expert').innerHTML = 'No winners yet!'}
}